...
from src.exceptions import GenerationError, RetrievalError, RoutingError, RAGPipelineError
...
class WorkflowState(BaseModel):
    ...
    # Routing state
    datasource: Optional[DataSource] = Field(None, description="Selected data source (arxiv or vectorstore)")
    routing_confidence: Optional[float] = Field(None, description="Confidence in routing decision")
    
    # Query processing state
    rewritten_query: Optional[str] = Field(None, description="Optimized version of the query")
    query_confidence: Optional[float] = Field(None, description="Confidence in query rewrite")
    
    # Retrieval state
    documents: List[str] = Field(default_factory=list, description="Retrieved documents")
    reranked_documents: List[str] = Field(default_factory=list, description="Reranked documents")
    reranking_confidence: Optional[float] = Field(None, description="Confidence in document ranking")
    
    # Response state
    response: Optional[str] = Field(None, description="Generated response")
    is_hallucination: Optional[bool] = Field(None, description="Whether response contains hallucinations")
    response_confidence: Optional[float] = Field(None, description="Confidence in response quality")
...
def invoke(user_input: str) -> dict:
    ...
    try:
        ...
        response_logger.info("Response: %s", output.get("response", "No response generated"))
        return output
        ...
        if "routing" in str(e).lower():
            raise RoutingError("Failed to route question", details={"query": user_input})
        elif "retrieval" in str(e).lower():
            raise RetrievalError("Failed to retrieve relevant documents", details={"query": user_input})
        else:
            raise GenerationError("Failed to generate response", details={"query": user_input})
